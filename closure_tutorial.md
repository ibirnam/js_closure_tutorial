...
This is why in question 1 from the last chapter, `localFn()` returned `localfoo` and not `foo`, since there was a local `foo` variable that lived within its own scope, it didn't look further up to check out the global variables. **If variables have the same name, the most local variable will always have the highest priority**.
...
* If `fight()` is called when `stamina` is <= 0, it should return a string of `"gotta run!"`
...
So we made some functions for Spider-Man to do things, but how can we web swing or fight Green Goblin if we don't have an actual Spider-Man to do that? A Spider-Man game requires a Spider-Man character? As the game's creater, you want to have a Spider-Man so that you can make use of the functions you've already written.
...
* He needs to be able to shoot webs
* He needs to be able to jump

We should write the Spider-Man variable as a closure! By doing so, we will be able to create **state** for our character. State will allow us to save Spider-Man's data (like his secret identity) through the entire lifetime of our game. It also allows us to update data and have those updates persist.
...
### Health, Stamina, and Webbing
These also should be variables we need to keep track of:
...
This seems good, but there's still a big problem with our code. None of the functions we wrote will be callable, as we did not store them anywhere!
...
* Electro has to have a secret identity, health, stamina, and charge level stats
* Electro should have a `fight` function that randomly picks between a punch, kick, or electric shock
	...
	* electrick shock requires 3 stamina and 5 charge
* Electro can also fire off an electric storm at will, which requires 10 stamina and 15 charge
* If the `fight()` or `storm()` functions are called and `stamina` is <= 0, Electro should say `"I need more juice"`
...
* There needs to be a public way to make Electro to fight
